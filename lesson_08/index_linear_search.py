...
# Створення індексної таблиці з кроком 3
index_table = create_index_table(main_array, 3)


# Функція для індексно-послідовного пошуку
def indexed_sequential_search(array, index_table, target):  # O(log(n)) + O(m) = O(log(n) + m)
    # Пошук в індексній таблиці
    start = 0   # це наш low який буде вказувати на upper_bound, якщо не знайдемо значення
